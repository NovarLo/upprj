...
*
*
*	Copyright (C), 2018-2020, 泰新博坤智能科技 http://www.htachina.com/
*
*********************************************************************************************************
*/
...
static void bsp_InitKeyHard(void)
{
	...
	/* 第1步：打开GPIO时钟 */
	__GPIOA_CLK_ENABLE();   // 目前只有GPIOA

	/* 第2步：配置所有的按键GPIO为浮动输入模式(实际上CPU复位后就是输入状态) */
	/* Configure GPIO pin : PA1 */
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;     /* 设为输入口 */
	GPIO_InitStruct.Alternate = GPIO_MODE_AF_PP; /* 设为推挽模式 */
	GPIO_InitStruct.Pull = GPIO_NOPULL;         /* 无需上下拉电阻 */
	GPIO_InitStruct.Speed = GPIO_SPEED_HIGH;    /* IO口最大速度 */
...
static void bsp_InitKeyVar(void)
{
	...
	s_tBtn[KID_JOY_U].RepeatSpeed = 10;  /* 每隔50ms自动发送键值 */

	s_tBtn[KID_JOY_D].LongTime = 200;  
	s_tBtn[KID_JOY_D].RepeatSpeed = 10;  /* 每隔50ms自动发送键值 */

	s_tBtn[KID_JOY_L].LongTime = 200;  
	s_tBtn[KID_JOY_L].RepeatSpeed = 10;  /* 每隔50ms自动发送键值 */

	s_tBtn[KID_JOY_R].LongTime = 200;  
	s_tBtn[KID_JOY_R].RepeatSpeed = 10;  /* 每隔50ms自动发送键值 */
...
/*****************************泰新博坤智能科技 http://www.htachina.com/ (END OF FILE) *******************************/
