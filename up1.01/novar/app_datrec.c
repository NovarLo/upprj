...
uint8_t NAND_Init(void)
{
	...
	FSMC_NAND_Ini();			/*配置FSMC和GPIO用于NAND Flash接口 */
...
uint8_t NAND_DirectWrite(uint32_t _ulMemAddr, uint32_t *_pWriteBuf, uint16_t _usSize)
{
	...
	/*　如果写入地址不是全0xFF, 且首地址位于该块的第0页和偏移地址0，则可以直接擦除 */
...
*			【扫描测试算法】
*			1) 第1个块（包括主数据区和备用数据区），擦除后检测是否全0xFF, 正确的话继续测试该块，否则该块
				是坏块,函数返回
...
*	函 数 名: DATREC_EXC
*	功能说明: 记录黑匣子数据
*   形    参:  	uint32_t *ptr —— 黑匣子数据写入指针，为BLOCKBAD_SIZE的整数倍，当前一条记录为256Bytes
*   							   该指针指向地址为NAND Flash统一编址码，范围0 ~ 1024 * 64 * 2048
*	返 回 值: 执行结果：
*				- NAND_FAIL 表示失败
*				- NAND_OK 表示成功
*********************************************************************************************************
...
uint8_t DATREC_EXC(uint32_t *ptr)
{
	...
	// 判断坏块数量是否超过24块，如果超出直接退出
	

	// 判断当前偏移地址确定写指针是否为当前块首地址
	...
	if (pageno == 0 && offset == 0)
	{
		...
		{
			*ptr = (blkno + 1) * NAND_PAGE_SIZE * NAND_BLOCK_SIZE;  // 跳到下一个块首地址
			return NAND_FAIL;
		}
		else
		{
	...
	{
		*ptr = (blkno + 1) * NAND_PAGE_SIZE * NAND_BLOCK_SIZE;  // 跳到下一个块首地址
		NAND_MarkBadBlock(blkno);	/*新块标记为坏块 */
		return NAND_FAIL;	/*读NAND Flash失败 */
	}
...
*/
	{
		...
		for (i = 0; i < BUFFER_SIZE; i++)	//填充数据以测试
		{
			TxBuffer[i] = i;
			RxBuffer[i] = 0;
		}
